...
class ModalSubmitInteraction(Interaction):
    ...
    def response(self) -> ModalSubmitInteractionResponse:
        """:class:`ModalSubmitInteractionResponse`: Returns an object responsible for handling responding to the interaction.

        A response can only be done once. If secondary messages need to be sent, consider using :attr:`followup`
        instead.
        """
        return ModalSubmitInteractionResponse(self)

    async def edit(self, *args, **kwargs) -> Optional[Message]:
        """|coro|

        This is a shorthand function for helping in editing messages in
        response to a modal submit interaction. If the
        interaction has not been responded to, :meth:`ModalSubmitInteractionResponse.edit_message`
        is used. If the response :meth:`~InteractionResponse.is_done` then
        the message is edited via the :attr:`ModalSubmitInteraction.message` using
        ...
        HTTPException
            Editing the message failed.
        InvalidArgument
            :attr:`ModalSubmitInteraction.message` was ``None``, this may occur in a :class:`Thread`
            or when the interaction is not created through a component interaction.

        Returns
        -------
        Optional[:class:`Message`]
            The edited message. If the interaction has not yet been responded to,
            :meth:`ModalSubmitInteractionResponse.edit_message` is used which returns
            a :class:`Message` or ``None`` corresponding to :attr:`ModalSubmitInteraction.message`.
        ...
        raise InvalidArgument(
            "ModalSubmitInteraction.message is None, this method can only be used after "
            "a response message has been sent, or if the interaction originates from a component interaction."
        )
    ...
    async def send(
        ...
        *,
        ...
        :meth:`ModalSubmitInteractionResponse.send_message` is used. If the response
    ...
    async def delete_original_message(self, *, delay: Optional[float] = None) -> None:
        ...
        if delay is not None:
            ...
            asyncio.create_task(inner_call())  # noqa: RUF006
        else:
            await delete_func
    ...
    async def original_message(self) -> InteractionMessage:
        ...
        If the interaction response was :meth:`ModalSubmitInteractionResponse.send_message` then this would
...
class ModalSubmitInteractionResponse(InteractionResponse):
    ...
    async def send_message(
        ...
        *,
        ...
        HTTPException
            Sending the message failed.
        NotFound
            The interaction has expired. :meth:`ModalSubmitInteractionResponse.defer` and
        ...
        :class:`PartialInteractionMessage`
            ...
            operations. To fetch the :class:`InteractionMessage` you may use :meth:`PartialInteractionMessage.fetch`
            or :meth:`ModalSubmitInteraction.original_message`.
        """
        if self._responded:
            raise InteractionResponded(self._parent)
    ...
    async def edit_message(
        ...
        *,
        ...
        modal submit interaction originated from.
        ...
        attachments: List[:class:`Attachment`]
            ...
            pass ``ModalSubmitInteraction.message.attachments``.
        view: Optional[:class:`~nextcord.ui.View`]
            The updated view to update this message with. If ``None`` is passed then
            the view is removed.
        delete_after: Optional[:class:`float`]
        ...
        -------
        Optional[:class:`Message`]
            The message that was edited, or None if the :attr:`ModalSubmitInteraction.message` is not found
